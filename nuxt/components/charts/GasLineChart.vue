...
const chartOptions = computed(() => {
    ...
    for (const measurement of measurements) {
        ...
        average.push(measurement.gasAvg);
        max.push(measurement.gasMax);
        min.push(measurement.gasMin);
    }
...
const percentageDifference = computed(() => {
    ...
    return getPercentageDifference(measurements[0]?.gasAvg, measurements.at(-1)?.gasAvg);
});
</script>

<template>
    <ChartCard title="Gas" :percentage="percentageDifference" :measurements="props.measurementsList" chart-type="line" :chartSeries="chartOptions.series" :chart-options="chartOptions" />
</template>