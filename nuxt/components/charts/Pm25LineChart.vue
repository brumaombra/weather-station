...
const chartOptions = computed(() => {
    ...
    for (const measurement of measurements) {
        ...
        average.push(measurement.pm25Avg);
        max.push(measurement.pm25Max);
        min.push(measurement.pm25Min);
    }
...
const percentageDifference = computed(() => {
    ...
    return getPercentageDifference(measurements[0]?.pm25Avg, measurements.at(-1)?.pm25Avg);
});
</script>

<template>
    <ChartCard title="PM2.5" :percentage="percentageDifference" :measurements="props.measurementsList" chart-type="line" :chartSeries="chartOptions.series" :chart-options="chartOptions" />
</template>