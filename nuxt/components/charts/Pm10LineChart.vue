...
const percentageDifference = computed(() => {
    ...
    return getPercentageDifference(measurements[0]?.pm10Avg, measurements.at(-1)?.pm10Avg);
});
</script>

<template>
    <ChartCard title="PM10" :percentage="percentageDifference" :measurements="props.measurementsList" chart-type="line" :chartSeries="chartOptions.series" :chart-options="chartOptions" />
</template>