...
const validateForm = computed(() => {
    return viewModel.username?.trim() !== '' && viewModel.password?.trim() !== '';
});
...
onMounted(() => {
    if (GlobalStore.loggedIn) {
        window.location.href = '/'; // If already logged in, redirect
    }
});
</script>
                        ...
                        <div class="mt-5">
                            <div>
                                <div class="grid gap-y-4">
                                    ...
                                    <!-- Login button -->
                                    <button @click="handleLoginPress" :disabled="!validateForm" class="mt-2 w-full py-3 px-4 inline-flex justify-center items-center gap-x-2 text-sm font-extrabold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50 disabled:pointer-events-none">Login</button>
                                </div>
                            </div>
                        </div>
                    </div>
